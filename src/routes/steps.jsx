...
export const Route = createFileRoute("/steps")({
    head: () => ({
        meta: [
            ...
            {
                name: "description",
                content: "Discover the five key steps of every election: voter registration, campaigning, voting day, vote counting, and result declaration.",
            },
    ...
    return (<Layout>
      <section className="relative mx-auto max-w-6xl px-4 py-16 sm:px-6">
        ...
        <ol className="mt-14 grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
          {steps.map((s, i) => (<li key={s.title} className="group relative overflow-hidden rounded-2xl border border-border bg-card p-6 shadow-[var(--shadow-card)] transition-all hover:-translate-y-1 hover:border-primary/30 hover:shadow-[var(--shadow-soft)]">
              <div className={`pointer-events-none absolute -right-12 -top-12 h-40 w-40 rounded-full bg-gradient-to-br ${s.tone} blur-2xl opacity-70 transition-opacity group-hover:opacity-100`} aria-hidden/>
